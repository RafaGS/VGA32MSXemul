...
byte PriSlotLReg,SSLReg[4];   /* Storage for A8h port and (FFFFh) */
...
Z80_WORD JoyState;                     /* Joystick states        */
...
Z80_WORD FMPACKey;                     /* MAGIC = SRAM active    */
...
Z80_WORD VAddr;                        /* VRAM address in VDP    */
...
void MapROM(Z80_WORD A,byte V);       /* Switch MegaROM banks            */
...
Z80_WORD SetIRQ(byte IRQ);            /* Set/Reset IRQ                   */
Z80_WORD StateID(void);               /* Compute emulation state ID      */
...
int StartMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
  ...
  /* *** 중요: MemMap 전체를 EmptyRAM으로 초기화 *** */
  if(Verbose) printf("Initializing memory map with EmptyRAM...\n");
  for(I=0;I<4;++I)
    for(J=0;J<4;++J)
      for(K=0;K<8;++K)
        MemMap[I][J][K]=EmptyRAM;

  /* *** RAM 배열도 EmptyRAM으로 초기화 *** */
  if(Verbose) printf("Initializing RAM pointers...\n");
  for(I=0;I<8;++I)
    RAM[I]=EmptyRAM;
  ...
  if(Verbose) printf("Calling ResetMSX...\n");
  if((ResetMSX(NewMode,NewRAMPages,NewVRAMPages)^NewMode)&MSX_MODEL) {
    if(Verbose) printf("ResetMSX failed!\n");
    return(0);
  }
  if(!RAMPages||!VRAMPages) {
    if(Verbose) printf("RAM or VRAM allocation failed!\n");
    return(0);
  }

  if(Verbose) printf("ResetMSX completed successfully\n");
...
int ResetMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
  ...
  // *** 중요: 메모리 맵 초기화 추가 ***
  // ResetMSX 시작 시 모든 포인터를 안전하게 초기화
  if(Verbose) printf("ResetMSX: Initializing memory maps...\n");
  
  // EmptyRAM이 없으면 초기화 실패
  if(!EmptyRAM) {
    if(Verbose) printf("ResetMSX: ERROR - EmptyRAM not allocated!\n");
    return(Mode);
  }
    ...
    switch(NewMode&MSX_MODEL)
    {
      case MSX_MSX1:
        ...
        if(!P1) {
          ...
          if(Verbose) printf("  ERROR: Failed to load MSX.ROM\n");
        }
        else
        {
          // 안전하게 메모리 해제
          if(MemMap[0][0][0] && MemMap[0][0][0]!=EmptyRAM) FreeMemory(MemMap[0][0][0]);
          if(MemMap[3][1][0] && MemMap[3][1][0]!=EmptyRAM) FreeMemory(MemMap[3][1][0]);
      ...
      case MSX_MSX2:
        ...
        if(!P1||!P2) 
        {
          ...
          if(P1) FreeMemory(P1);
          if(P2) FreeMemory(P2);
          if(Verbose) printf("  ERROR: Failed to load MSX2 ROMs\n");
        }
        else
        {
          if(MemMap[0][0][0] && MemMap[0][0][0]!=EmptyRAM) FreeMemory(MemMap[0][0][0]);
  ...
  if((Mode^NewMode)&MSX_PATCHBDOS)
  {
    ...
    if(!P1) {
      ...
      if(Verbose) printf("  ERROR: Failed to load DISK.ROM\n");
    }
    else
    {
      /* Assign new DiskROM */
      if(MemMap[3][1][2] && MemMap[3][1][2]!=EmptyRAM) FreeMemory(MemMap[3][1][2]);
 ...
 /* Correct RAM and VRAM sizes */
  ...
  // *** 중요: RAMData와 VRAM이 NULL이 아닌지 확인 ***
  if(!RAMData || !VRAM) {
    if(Verbose) printf("ResetMSX: ERROR - RAMData or VRAM is NULL!\n");
    return(Mode);
  }
  ...
  for(J=0;J<4;++J)
  {
    ...
    /* RAMMap=3:2:1:0 - 안전하게 초기화 */
    int ramOffset = (3-J)*0x4000;
    if(ramOffset < RAMPages*0x4000) {  // 범위 체크
      MemMap[3][2][J*2]   = RAMData+ramOffset;
      MemMap[3][2][J*2+1] = MemMap[3][2][J*2]+0x2000;
      RAMMapper[J]        = 3-J;
    } else {
      // 범위를 벗어나면 EmptyRAM 사용
      MemMap[3][2][J*2]   = EmptyRAM;
      MemMap[3][2][J*2+1] = EmptyRAM;
      RAMMapper[J]        = 0;
    }
    ...
    // NULL 포인터 체크
    if(!RAM[J*2]) RAM[J*2] = EmptyRAM;
    if(!RAM[J*2+1]) RAM[J*2+1] = EmptyRAM;
  }
 /* For all slots... */
  if(Verbose) printf("Initializing slots...\n");
  ...
  for(J=0;J<4;++J)
  {
    ...
    /* RAMMap=3:2:1:0 - 안전하게 초기화 */
    int ramPage = 3-J;
    int ramOffset = ramPage * 0x4000;
    
    // MemMap[3][2] 초기화 (RAM 슬롯)
    if(RAMData && (ramOffset < RAMPages*0x4000)) {
      MemMap[3][2][J*2]   = RAMData + ramOffset;
      MemMap[3][2][J*2+1] = MemMap[3][2][J*2] + 0x2000;
      RAMMapper[J]        = ramPage;
      if(Verbose) printf("  Slot %d: RAM page %d at offset 0x%X (0x%p, 0x%p)\n", 
                        J, ramPage, ramOffset, 
                        MemMap[3][2][J*2], MemMap[3][2][J*2+1]);
    } else {
      MemMap[3][2][J*2]   = EmptyRAM;
      MemMap[3][2][J*2+1] = EmptyRAM;
      RAMMapper[J]        = 0;
      if(Verbose) printf("  Slot %d: Using EmptyRAM\n", J);
    }
    
    /* Setting address space from MemMap[0][0] (BIOS) */
    // 초기 상태에서는 PSL[J]=0, SSL[J]=0이므로 MemMap[0][0]을 사용
    RAM[J*2]   = MemMap[0][0][J*2]   ? MemMap[0][0][J*2]   : EmptyRAM;
    RAM[J*2+1] = MemMap[0][0][J*2+1] ? MemMap[0][0][J*2+1] : EmptyRAM;
    
    if(Verbose) printf("  RAM[%d]=0x%p, RAM[%d]=0x%p\n", 
                      J*2, RAM[J*2], J*2+1, RAM[J*2+1]);
  }

  if(Verbose) printf("Slots initialized\n");

  /* For all MegaROMs... */
  if(Verbose) printf("Initializing MegaROMs...\n");
  
  for(J=0;J<MAXSLOTS;++J)
  {
    if(!ROMData[J]) continue;
    
    I = ROMMask[J]+1;
    if(I>4)
    {
      if((ROMData[J][0]=='A')&&(ROMData[J][1]=='B')) {
        if(Verbose) printf("  MegaROM %d: Setting pages 0,1,2,3\n", J);
        SetMegaROM(J,0,1,2,3);
      }
      else if((ROMData[J][(I-2)<<13]=='A')&&(ROMData[J][((I-2)<<13)+1]=='B')) {
        if(Verbose) printf("  MegaROM %d: Setting pages %d,%d,%d,%d\n", 
                          J, I-2, I-1, I-2, I-1);
  ...
  if(Verbose) printf("MegaROMs initialized\n");

  /* Reset sound chips */
  if(Verbose) printf("Resetting sound chips...\n");
  ...
  if(Verbose) printf("Sound chips reset\n");

  /* Reset serial I/O */
  if(Verbose) printf("Resetting serial I/O...\n");
  Reset8251(&SIO,ComIStream,ComOStream);
  if(Verbose) printf("Serial I/O reset\n");

  /* Reset PPI chips and slot selectors */
  if(Verbose) printf("Resetting PPI...\n");
  Reset8255(&PPI);
  ...
  SSLReg[2]=0x00;
  SSLReg[3]=0x00;
  if(Verbose) printf("PPI reset\n");

  /* Reset floppy disk controller */
  if(Verbose) printf("Resetting FDC...\n");
  Reset1793(&FDC,FDD,WD1793_KEEP);
  if(Verbose) printf("FDC reset\n");

  /* Reset VDP */
  if(Verbose) printf("Resetting VDP...\n");
  memcpy(VDP,VDPInit,sizeof(VDP));
  memcpy(VDPStatus,VDPSInit,sizeof(VDPStatus));
  if(Verbose) printf("VDP reset\n");

  /* Reset keyboard */
  if(Verbose) printf("Resetting keyboard...\n");
  memset((void *)KeyState,0xFF,16);
  if(Verbose) printf("Keyboard reset\n");

  /* Set initial palette */
  if(Verbose) printf("Setting palette...\n");
  for(J=0;J<16;++J)
  {
    Palette[J]=PalInit[J];
    SetColor(J,(Palette[J]>>16)&0xFF,(Palette[J]>>8)&0xFF,Palette[J]&0xFF);
  }
  if(Verbose) printf("Palette set\n");

  /* Reset mouse coordinates/counters */
  if(Verbose) printf("Resetting mouse...\n");
  for(J=0;J<2;++J)
  ...
  if(Verbose) printf("Setting VDP tables...\n");
  ...
  if(Verbose) printf("VDP tables set\n");

  /* Set "V9958" VDP version for MSX2+ */
  if(MODEL(MSX_MSX2P)) VDPStatus[1]|=0x04;

  /* Reset CPU */
  if(Verbose) printf("Resetting Z80 CPU...\n");
  ResetZ80(&CPU);
  if(Verbose) printf("Z80 CPU reset\n");
...
/** given I/O port.                                         **/
/*************************************************************/
void OutZ80(Z80_WORD Port,byte Value)
{
  register byte I,J;
  ...
  else
  {
    ...
    switch(Value&0xC0)
    {
      ...
      case 0x40:
        ...
        VAddr=(((Z80_WORD)Value<<8)+ALatch)&0x3FFF;
...
/*************************************************************/
void MapROM(register Z80_WORD A,register byte V)
{
byte I,J,PriSlot,SS,*P;
...
PriSlot = PSL[J];          /* Primary slot number   */
  SS = SSL[J];          /* Secondary slot number */
  I  = CartMap[PriSlot][SS]; /* Cartridge number      */
  ...
  switch(ROMType[I])
  {
    case MAP_GEN8: /* Generic 8kB cartridges (Konami, etc.) */
      ...
      if(V!=ROMMapper[I][J])
      {
        RAM[J+2]=MemMap[PriSlot][SS][J+2]=ROMData[I]+((int)V<<13);
        ROMMapper[I][J]=V;
      }
      if(Verbose&0x08)
        printf("ROM-MAPPER %c: 8kB ROM page #%d at %d:%d:%04Xh\n",I+'A',V,PriSlot,SS,J*0x2000+0x4000);
      return;
    ...
    case MAP_GEN16: /* Generic 16kB cartridges (MSXDOS2, HoleInOneSpecial) */
      ...
      {
        RAM[J+2]=MemMap[PriSlot][SS][J+2]=ROMData[I]+((int)V<<13);
        RAM[J+3]=MemMap[PriSlot][SS][J+3]=RAM[J+2]+0x2000;
        ROMMapper[I][J]=V;
        ROMMapper[I][J+1]=V|1;
      }
      if(Verbose&0x08)
        printf("ROM-MAPPER %c: 16kB ROM page #%d at %d:%d:%04Xh\n",I+'A',V>>1,PriSlot,SS,J*0x2000+0x4000);
      return;
    ...
    case MAP_ASCII8: /* ASCII 8kB cartridges */
      ...
      if((A>=0x6000)&&(A<0x8000))
      {
        ...
        if(V!=ROMMapper[I][J])
        {
          MemMap[PriSlot][SS][J+2]=P;
          ROMMapper[I][J]=V;
          /* Only update memory when cartridge's slot selected */
          if((PSL[(J>>1)+1]==PriSlot)&&(SSL[(J>>1)+1]==SS)) RAM[J+2]=P;
        }
    ...
    case MAP_ASCII16: /*** ASCII 16kB cartridges ***/
      ...
      if((A>=0x6000)&&(A<0x8000)&&((V<=ROMMask[I]+1)||!(A&0x0FFF)))
      {
        ...
        {
          MemMap[PriSlot][SS][J+2]=P;
          MemMap[PriSlot][SS][J+3]=P+0x2000;
          ROMMapper[I][J]=V;
          ROMMapper[I][J+1]=V|1;
          /* Only update memory when cartridge's slot selected */
          if((PSL[(J>>1)+1]==PriSlot)&&(SSL[(J>>1)+1]==SS))
          {
            RAM[J+2]=P;
            RAM[J+3]=P+0x2000;
          }
    ...
    case MAP_GMASTER2: /* Konami GameMaster2+SRAM cartridge */
      ...
      if((A>=0x6000)&&(A<=0xA000)&&!(A&0x1FFF))
      {
        ...
        {
          /* Select SRAM page */
          RAM[J+2]=MemMap[PriSlot][SS][J+2]=SRAMData[I]+(V&0x20? 0x2000:0);
          /* SRAM is now on */
          ROMMapper[I][J]=0xFF;
          if(Verbose&0x08)
            printf("GMASTER2 %c: 4kB SRAM page #%d at %d:%d:%04Xh\n",I+'A',(V&0x20)>>5,PriSlot,SS,J*0x2000+0x4000);
        }
        else
        {
    ...
    case MAP_FMPAC: /* Panasonic FMPAC+SRAM cartridge */
      ...
      switch(A)
      {
        case 0x7FF7: /* ROM page select */
          ...
          {
            P=ROMData[I]+((int)V<<13);
            RAM[2]=MemMap[PriSlot][SS][2]=P;
            RAM[3]=MemMap[PriSlot][SS][3]=P+0x2000;
          }
          if(Verbose&0x08)
...
/*************************************************************/
Z80_WORD SetIRQ(register byte IRQ)
{
...
void SetMegaROM(int Slot,byte P0,byte P1,byte P2,byte P3)
{
  ...
  for(PriSlot=0;PriSlot<4;++PriSlot)
  {
    for(SS=0;(SS<4)&&(CartMap[PriSlot][SS]!=Slot);++SS);
    if(SS<4) break;
  }
  ...
  P3&=ROMMask[Slot];
  /* Set memory map */
  MemMap[PriSlot][SS][2]=ROMData[Slot]+P0*0x2000;
  MemMap[PriSlot][SS][3]=ROMData[Slot]+P1*0x2000;
  MemMap[PriSlot][SS][4]=ROMData[Slot]+P2*0x2000;
  MemMap[PriSlot][SS][5]=ROMData[Slot]+P3*0x2000;
  /* Set ROM mappers */
  ROMMapper[Slot][0]=P0;
  ROMMapper[Slot][1]=P1;
  ROMMapper[Slot][2]=P2;
    ...
    for(S=SprTab;J;J>>=1,S+=4)
      if(J&1)
            ...
            if((DV<16)||(DV>240))
	    {
              ...
              if((DH<16)||(DH>240))
	      {
                PriSlot=SprGen+((int)(S[2]&0xFC)<<3);
                PD=SprGen+((int)(D[2]&0xFC)<<3);
                if(DV<16) PD+=DV; else { DV=256-DV;PriSlot+=DV; }
                if(DH>240) { DH=256-DH;T=PriSlot;PriSlot=PD;PD=T; }
                while(DV<16)
                {
                  LS=((unsigned int)*PriSlot<<8)+*(PriSlot+16);
        ...
        for(I=J>>1,D=S+4;I;I>>=1,D+=4)
          if(I&1) 
          {
            ...
            if((DV<8)||(DV>248))
            {
              ...
              if((DH<8)||(DH>248))
              {
                PriSlot=SprGen+((int)S[2]<<3);
                PD=SprGen+((int)D[2]<<3);
                if(DV<8) PD+=DV; else { DV=256-DV;PriSlot+=DV; }
                if(DH>248) { DH=256-DH;T=PriSlot;PriSlot=PD;PD=T; }
                while((DV<8)&&!(*PD&(*PriSlot>>DH))) { ++DV;++PriSlot;++PD; }
                if(DV<8) return(1);
              }
            }
          }
...
byte *LoadROM(const char *Name,int Size,byte *Buf)
{
  ...
  if(!P)
  {
    if(Verbose) printf("Failed to allocate %d bytes\n", Size);
    fclose(F);
    return(0);
  }
  ...
  if(J!=Size)
  {
    if(Verbose) printf("Read %d bytes, expected %d\n", J, Size);
    if(!Buf) FreeMemory(P);
    fclose(F);
    return(0);
  }
...
int LoadCart(const char *FileName,int Slot,int Type)
{
  ...
  if(!FileName)
  {
    if(ROMData[Slot])
    {
      ...
      for(C1=0;C1<8;++C1) MemMap[PriSlot][SS][C1]=EmptyRAM;
      /* Restart MSX */
  ...
  switch(Len)
  {
    case 1:
      ...
      if(!BASIC)
      {
        MemMap[PriSlot][SS][0]=P;
        MemMap[PriSlot][SS][1]=P;
        MemMap[PriSlot][SS][2]=P;
        MemMap[PriSlot][SS][3]=P;
      }
      MemMap[PriSlot][SS][4]=P;
      MemMap[PriSlot][SS][5]=P;
      if(!BASIC)
      {
        MemMap[PriSlot][SS][6]=P;
        MemMap[PriSlot][SS][7]=P;
      }
      break;
    ...
    case 2:
      ...
      {
        MemMap[PriSlot][SS][0]=P;
        MemMap[PriSlot][SS][1]=P+0x2000;
        MemMap[PriSlot][SS][2]=P;
        MemMap[PriSlot][SS][3]=P+0x2000;
      }
      MemMap[PriSlot][SS][4]=P;
      MemMap[PriSlot][SS][5]=P+0x2000;
      if(!BASIC)
      {
        MemMap[PriSlot][SS][6]=P;
        MemMap[PriSlot][SS][7]=P+0x2000;
      }
      break;
    ...
    case 4:
      ...
      MemMap[PriSlot][SS][2]=P;
      MemMap[PriSlot][SS][3]=P+0x2000;
      MemMap[PriSlot][SS][4]=P+0x4000;
      MemMap[PriSlot][SS][5]=P+0x6000;
      MemMap[PriSlot][SS][6]=P+0x4000;
      MemMap[PriSlot][SS][7]=P+0x6000;
      break;
    ...
    default:
      if(ROM64)
      {
        ...
        MemMap[PriSlot][SS][0]=P;
        MemMap[PriSlot][SS][1]=P+0x2000;
        MemMap[PriSlot][SS][2]=P+0x4000;
        MemMap[PriSlot][SS][3]=P+0x6000;
        MemMap[PriSlot][SS][4]=P+0x8000;
        MemMap[PriSlot][SS][5]=P+0xA000;
        MemMap[PriSlot][SS][6]=P+0xC000;
        MemMap[PriSlot][SS][7]=P+0xE000;
      }
      break;
  }
  ...
  if(Verbose)
    ...
    (
      "starts at %04Xh..",
      MemMap[PriSlot][SS][2][2]+256*MemMap[PriSlot][SS][2][3]
    );
